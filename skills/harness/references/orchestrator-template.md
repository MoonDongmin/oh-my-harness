...
2. **에이전트 팀 모드에서는 올바른 도구 호출 순서를 준수**:
   - `TeamCreate(team_name: "...")` — team_name만 전달 (members 파라미터 없음)
   - `Agent(subagent_type: "...", name: "...", team_name: "...", model: "opus", prompt: "...")` — 각 팀원을 개별 Agent 도구로 스폰
   - `TaskCreate(subject: "...", description: "...")` — 작업을 하나씩 등록
   - `SendMessage(to: "...", message: "...")` — 팀원 간 통신
...
`references/harness-examples.md`의 리서치 팀 예시를 참조.
